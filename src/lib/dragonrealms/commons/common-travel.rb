# Module for DragonRealms common travel and merchant interaction functionality
#
...
module Lich
  module DragonRealms
    module DRCT
      ...
      # Visits a merchant and attempts to sell an item
      #
      # @param room [Integer] The room ID where the merchant is located
      # @param item [String] The item to sell
      # @return [Boolean] true if sale successful, false if failed
      # @example
      #   DRCT.sell_item(1234, "steel sword")
      #
      # @note Item must be in hands to sell. Common use is selling junk at pawn shops.
      ...
      # Visits a merchant and purchases an item by offering their asking price
      #
      # @param room [Integer] The room ID where the merchant is located
      # @param item [String] The item to buy
      # @return [void]
      # @example
      #   DRCT.buy_item(1234, "crossbow bolt")
      #
      # @note Used for restocking ammunition and other misc items
      ...
      # Visits an NPC and asks them for a specific item
      #
      # @param room [Integer] The room ID where the NPC is located
      # @param name [String] The name of the NPC to ask
      # @param item [String] The item to request
      # @return [Boolean] true if item received, false if request failed
      # @example
      #   DRCT.ask_for_item?(1234, "shopkeeper", "rope")
      #
      # @note Commonly used for bundling ropes or gem pouches
      ...
      # Orders an item from a merchant's menu by number
      #
      # @param room [Integer] The room ID where the merchant is located
      # @param item_number [Integer] The menu number of the item to order
      # @return [void]
      # @raise [RuntimeError] If insufficient funds
      # @example
      #   DRCT.order_item(1234, 5)
      ...
      # Disposes of an item either in a room or worn container
      #
      # @param item [String] The item to dispose of
      # @param trash_room [Integer, nil] Optional room ID with disposal location
      # @param worn_trashcan [String, nil] Optional worn container to put trash in
      # @param worn_trashcan_verb [String, nil] Optional verb for using worn container
      # @return [void]
      # @example
      #   DRCT.dispose("broken sword", 1234, "backpack", "put")
      ...
      # Refills a lockpick container with new lockpicks
      #
      # @param lockpick_type [String] The type of lockpick to buy
      # @param hometown [String] The current hometown
      # @param container [String] The container to store lockpicks
      # @param count [Integer] Number of lockpicks to buy
      # @return [void]
      # @example
      #   DRCT.refill_lockpick_container("iron", "Crossing", "lockpick ring", 5)
      #
      # @note Will not mix different lockpick types in same container
      ...
      # Navigates to a target room
      #
      # @param target_room [Integer, String] Room ID or tag to navigate to
      # @param restart_on_fail [Boolean] Whether to retry on failure
      # @return [Boolean] true if navigation successful, false if failed
      # @example
      #   DRCT.walk_to(1234)
      #   DRCT.walk_to("bank")
      #
      # @note Handles locked doors and combat engagement automatically
      ...
      # Converts a room tag to a room ID
      #
      # @param target [String] The room tag to convert
      # @return [Integer] The room ID matching the tag
      # @raise [SystemExit] If no matching room found
      # @example
      #   room_id = DRCT.tag_to_id("bank")
      ...
      # Retreats from NPCs in current room
      #
      # @param ignored_npcs [Array<String>] NPCs to ignore when retreating
      # @return [void]
      # @example
      #   DRCT.retreat(["shopkeeper", "guard"])
      ...
      # Finds an empty room from a list of possibilities
      #
      # @param search_rooms [Array<Integer>] Room IDs to search
      # @param idle_room [Integer, Array<Integer>] Room(s) to wait in if no empty rooms found
      # @param predicate [Proc, nil] Optional custom room validation
      # @param min_mana [Integer] Minimum mana level required (default: 0)
      # @param strict_mana [Boolean] Whether to strictly enforce mana requirement
      # @param max_search_attempts [Integer] Maximum search attempts
      # @param prioritize_buddies [Boolean] Whether to prefer rooms with friends
      # @return [Boolean] true if suitable room found, false if not
      # @example
      #   DRCT.find_empty_room([1234,5678], 1111, nil, 50)
      ...
      # Sorts a list of destinations by shortest path from current location
      #
      # @param target_list [Array<Integer>] List of room IDs to sort
      # @return [Array<Integer>] Sorted room IDs by distance
      # @example
      #   sorted = DRCT.sort_destinations([1234, 5678, 9012])
      ...
      # Finds an empty room from a sorted list of possibilities
      #
      # @param search_rooms [Array<Integer>] Room IDs to search
      # @param idle_room [Integer] Room to wait in if no empty rooms found
      # @param predicate [Proc, nil] Optional custom room validation
      # @return [Boolean] true if suitable room found, false if not
      # @example
      #   DRCT.find_sorted_empty_room([1234,5678], 1111)
      ...
      # @param destination [Integer] Ending room ID
      # @return [Integer] Estimated travel time in seconds
      # @example
      #   time = DRCT.time_to_room(1234, 5678)
      ...
      #
      # @param path [Array<String>] List of directions to reverse
      # @return [Array<String>] Reversed path directions
      # @raise [SystemExit] If invalid direction found
      # @example
      #   reverse = DRCT.reverse_path(["north", "east", "up"])
      ...
      # Gets a room ID for a target location in a specific hometown
      #
      # @param hometown [String] The hometown to look up
      # @param target [String] The target location type
      # @return [Integer, nil] Room ID if found, nil if not
      # @example
      #   bank_id = DRCT.get_hometown_target_id("Crossing", "bank")
      #
      # @note Will retry once on failure to load data
