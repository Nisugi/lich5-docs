...
module Lich
  module Common
    # A class responsible for parsing XML data related to game state and player attributes.
    class XMLParser
      # @return [Integer] the current mana of the player
      # @return [Integer] the maximum mana of the player
      # @return [Integer] the current health of the player
      # @return [Integer] the maximum health of the player
      # @return [Integer] the current spirit of the player
      # @return [Integer] the maximum spirit of the player
      # @return [Integer, nil] the last spirit value
      # @return [Integer] the current stamina of the player
      # @return [Integer] the maximum stamina of the player
      # @return [String] the text representation of the player's stance
      # @return [Integer] the value representation of the player's stance
      # @return [String] the text representation of the player's mind
      # @return [Integer] the value representation of the player's mind
      # @return [String] the prepared spell by the player
      # @return [String] the text representation of encumbrance
      # @return [String] the full text representation of encumbrance
      # @return [Integer] the value representation of encumbrance
      # @return [Hash] the indicator status
      # @return [Hash] the injuries sustained by the player
      # @return [Integer] the mode of injury
      # @return [Integer] the count of rooms visited
      # @return [String] the name of the current room
      # @return [String] the title of the current room
      # @return [String] the description of the current room
      # @return [Array] the exits available in the current room
      # @return [String] the string representation of room exits
      # @return [String] the title of the familiar room
      # @return [String] the description of the familiar room
      # @return [Array] the exits available in the familiar room
      # @return [String] the current bounty task
      # @return [Integer] the server time
      # @return [Integer] the server time offset
      # @return [Hash] the active spells
      # @return [Integer] the end time of roundtime
      # @return [Integer] the end time of cast roundtime
      # @return [Integer] the last pulse time
      # @return [Integer] the player's level
      # @return [Integer] the value for the next level
      # @return [String] the text for the next level
      # @return [String] the current society task
      # @return [Integer, nil] the stow container ID
      # @return [String] the name of the player
      # @return [String] the game name
      # @return [Boolean] the in_stream status
      # @return [String] the player ID
      # @return [String] the prompt for the player
      # @return [Array] the current target IDs
      # @return [Integer, nil] the current target ID
      # @return [Boolean] the room window disabled status
      # @return [Hash] the dialogs available
      # @return [Integer, nil] the room ID
      # @return [Integer, nil] the previous navigation room
      # @return [Integer] the current concentration
      # @return [Integer] the maximum concentration
      # @return [Array] the arrival PCs
      # @return [Boolean] the room player hidden status
      ...
      # @return [Boolean] whether to send fake tags
      ...
      # Initializes a new instance of XMLParser.
      # @note This constructor sets up various attributes to their default values.
      def initialize
        @buffer = String.new
      ...
      # Retrieves the active spells and their statuses.
      # @return [Hash] a hash containing active spells and their details
      # @example
      #   parser = XMLParser.new
      #   active_spells = parser.active_spells
      def active_spells
        z = {}
      ...
      # Resets the parser's state by clearing active tags and IDs.
      # @return [void]
      # @example
      #   parser.reset
      def reset
      ...
      # Checks if it is safe to respond based on the current game state.
      # @return [Boolean] true if safe to respond, false otherwise
      # @note This method checks the game prefix to determine the response safety.
      def safe_to_respond?
        if @game =~ /^DR/
      ...
      # Generates a binary string representation of the player's wounds.
      # @return [String] the wound GSL (Game State Language) representation
      # @example
      #   parser.make_wound_gsl
      def make_wound_gsl
      ...
      # Generates a binary string representation of the player's scars.
      # @return [String] the scar GSL (Game State Language) representation
      # @example
      #   parser.make_scar_gsl
      def make_scar_gsl
      ...
      # Parses the progress bar data from the PSM3 format.
      # @param kind [String] the type of progress bar (e.g., "Buffs", "Active Spells")
      # @param attributes [Hash] the attributes of the progress bar including id and text
      # @return [void]
      # @example
      #   parser.parse_psm3_progressbar("Buffs", { "id" => "1", "text" => "Buff Name", "time" => "00:30:00" })
      ...
      #
      # This method is called once per element by REXML in games.rb.
      # It processes various XML tags and updates the game state accordingly.
      #
      # @param name [String] the name of the XML tag being processed.
      # @param attributes [Hash] a hash of attributes associated with the XML tag.
      # @return [void]
      # @raise [StandardError] if an error occurs during processing.
      # @example
      #   tag_start('nav', { 'id' => '123', 'rm' => '456' })
      #
      # @note This method modifies several instance variables and may have side effects
      #       on the game state, including clearing game objects and updating player status.
      ...
      # Processes the given text string and updates the game state accordingly.
      #
      # This method is called once per element with text in it by REXML in games.rb.
      # 
      # @param text_string [String] The text string to process.
      # @return [void]
      # @raise [StandardError] If an error occurs during processing.
      # @example
      #   text("<stream id='Spells'>Some spell text</stream>")
      # 
      # @note This method handles various game states and updates internal variables
      #       based on the content of the text string. It also manages active spells,
      #       room descriptions, and player statuses.
      ...
      # Handles the end of a tag in the XML parsing process.
      #
      # This method is called once per element by REXML in games.rb.
      # It processes various tags and updates game state accordingly.
      #
      # @param name [String] the name of the tag that has ended
      # @return [void]
      # @raise [StandardError] if an error occurs during processing
      # @example
      #   tag_end('compass')
      #   # Processes the end of a 'compass' tag
      #
      # @note This method modifies global variables and game state.
      def tag_end(name)
        # This is called once per element by REXML in games.rb
        # https://ruby-doc.org/stdlib-2.6.1/libdoc/rexml/rdoc/REXML/StreamListener.html
      ...
      #
      # This method is here for backwards compatibility, but spellfront XML
      # isn't sent by the game anymore. It warns the user if the method is
      # called after a certain time interval.
      #
      # @return [Array] an array of active spell keys
      # @raise [StandardError] if an error occurs during retrieval
      # @example
      #   spells = spellfront
      #   # => returns an array of active spells
      #
      # @note This method will be removed in a future version of Lich.
      def spellfront
