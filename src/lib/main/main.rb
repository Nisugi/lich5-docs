...
# @note This proc handles reconnection logic if the appropriate command line arguments are provided.
# @return [void]
# @example
...
# @note This thread handles the main logic for launching the application and processing login.
# @return [void]
# @example
...
@main_thread = Thread.new {
  ...
  # Checks the launch data for game configuration and initializes the game environment.
  #
  # @return [void]
  # @raise [SystemExit] if any required game configuration is missing.
  # @example
  #   if @launch_data
  ...
  # Checks if the frontend is disabled and starts a new thread to handle client communication.
  # 
  # @return [void]
  # @example
  #   if ARGV.include?('--without-frontend')
  if ARGV.include?('--without-frontend')
    ...
    #
    # @return [void]
    # @note This thread will handle various frontends differently based on the value of `$frontend`.
  ...
  unless @detachable_client_port.nil?
    # Starts a new thread for handling a detachable client connection.
    #
    # @return [void]
    # @note This thread will continuously attempt to accept a connection on the specified host and port.
  ...
  # Waits while the offline mode is active.
  #
  # @return [void]
  # @example
  ...
  # Initializes highlight characters for the frontend wizard.
  # 
  # @note This section is only executed if the frontend is set to 'wizard'.
  if $frontend == 'wizard'
  ...
  # Sets the priority of the client thread.
  #
  # @param priority [Integer] The priority level to set for the client thread.
  client_thread.priority = 3

  # Sends a message to the client indicating that Lich is active.
  #
  # @return [void]
  # @example
  ...
  # Waits for the game thread to finish execution.
  #
  # @return [void]
  Game.thread.join
  ...
  # Logs the saving of script settings.
  #
  # @return [void]
  ...
  # Logs the closing of connections.
  #
  # @return [void]
  ...
  # Calls the reconnect procedure if needed.
  #
  # @return [void]
  # @note This is taken out of play but may need to see if anyone's using it.
  reconnect_if_wanted.call # taking this out of play but may need to see if anyone's using it
  
  # Logs the exiting of the application.
  #
  # @return [void]
  Lich.log "info: exiting..."
  Gtk.queue { Gtk.main_quit } if defined?(Gtk)
